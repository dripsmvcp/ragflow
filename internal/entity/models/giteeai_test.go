...
func newGiteeAIServer(t *testing.T, expectedPath string, handler func(t *testing.T, body map[string]interface{}, w http.ResponseWriter)) *httptest.Server {
	t.Helper()
...
func newGiteeAISSEServer(t *testing.T, expectedPath, ssePayload string) *httptest.Server {
	t.Helper()
	return httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodPost {
...
func TestGiteeAIName(t *testing.T) {
	if got := newGiteeAIForTest("http://unused").Name(); got != "giteeai" {
		t.Errorf("Name()=%q, want %q", got, "giteeai")
	}
}

func TestGiteeAIFactory(t *testing.T) {
...
func TestGiteeAIChatHappyPath(t *testing.T) {
	srv := newGiteeAIServer(t, "/chat/completions", func(t *testing.T, body map[string]interface{}, w http.ResponseWriter) {
...
func TestGiteeAIChatNoReasoning(t *testing.T) {
	srv := newGiteeAIServer(t, "/chat/completions", func(t *testing.T, body map[string]interface{}, w http.ResponseWriter) {
		_ = json.NewEncoder(w).Encode(map[string]interface{}{
			"choices": []map[string]interface{}{{
...
func TestGiteeAIChatRequiresAPIKey(t *testing.T) {
	_, err := newGiteeAIForTest("http://unused").ChatWithMessages(
		"gpt-4o-mini",
...
func TestGiteeAIChatRequiresMessages(t *testing.T) {
	apiKey := "test-key"
	_, err := newGiteeAIForTest("http://unused").ChatWithMessages(
...
func TestGiteeAIChatPropagatesHTTPError(t *testing.T) {
	srv := newGiteeAIServer(t, "/chat/completions", func(t *testing.T, body map[string]interface{}, w http.ResponseWriter) {
	...
	apiKey := "test-key"
	_, err := newGiteeAIForTest(srv.URL).ChatWithMessages(
		"gpt-4o-mini",
		[]Message{{Role: "user", Content: "x"}},
		&APIConfig{ApiKey: &apiKey}, nil)
...
func TestGiteeAIStreamHappyPath(t *testing.T) {
	srv := newGiteeAISSEServer(t, "/chat/completions",
...
func TestGiteeAIStreamSplitsReasoning(t *testing.T) {
	srv := newGiteeAISSEServer(t, "/chat/completions",
		`data: {"choices":[{"index":0,"delta":{"role":"assistant"}}]}`+"\n"+
...
func TestGiteeAIStreamHonorsFinishWithoutDelta(t *testing.T) {
...
func TestGiteeAIStreamRejectsExplicitFalse(t *testing.T) {
	apiKey := "test-key"
	stream := false
	err := newGiteeAIForTest("http://unused").ChatStreamlyWithSender(
		"gpt-4o-mini",
		[]Message{{Role: "user", Content: "x"}},
		&APIConfig{ApiKey: &apiKey},
...
func TestGiteeAIStreamRequiresSender(t *testing.T) {
	apiKey := "test-key"
	err := newGiteeAIForTest("http://unused").ChatStreamlyWithSender(
		"gpt-4o-mini",
		[]Message{{Role: "user", Content: "x"}},
...
func TestGiteeAIStreamFailsWithoutTerminal(t *testing.T) {
	srv := newGiteeAISSEServer(t, "/chat/completions",
...
func TestGiteeAIStreamRejectsMalformedFrame(t *testing.T) {
	srv := newGiteeAISSEServer(t, "/chat/completions",
...
func TestGiteeAIStreamSurfacesUpstreamError(t *testing.T) {
	srv := newGiteeAISSEServer(t, "/chat/completions",
...
func TestGiteeAIListModelsHappyPath(t *testing.T) {
	srv := newGiteeAIServer(t, "/models", func(t *testing.T, _ map[string]interface{}, w http.ResponseWriter) {
		_ = json.NewEncoder(w).Encode(map[string]interface{}{
	...
	apiKey := "test-key"
	models, err := newGiteeAIForTest(srv.URL).ListModels(&APIConfig{ApiKey: &apiKey})
	if err != nil {
...
func TestGiteeAIListModelsRequiresAPIKey(t *testing.T) {
	_, err := newGiteeAIForTest("http://unused").ListModels(&APIConfig{})
	if err == nil || !strings.Contains(err.Error(), "api key is required") {
		t.Errorf("expected api-key error, got %v", err)
	}
}

func TestGiteeAICheckConnectionDelegatesToListModels(t *testing.T) {
	srv := newGiteeAIServer(t, "/models", func(t *testing.T, _ map[string]interface{}, w http.ResponseWriter) {
		_ = json.NewEncoder(w).Encode(map[string]interface{}{
	...
	apiKey := "test-key"
	if err := newGiteeAIForTest(srv.URL).CheckConnection(&APIConfig{ApiKey: &apiKey}); err != nil {
		t.Errorf("CheckConnection: %v", err)
	}
}

func TestGiteeAICheckConnectionPropagatesError(t *testing.T) {
	srv := newGiteeAIServer(t, "/models", func(t *testing.T, _ map[string]interface{}, w http.ResponseWriter) {
		w.WriteHeader(http.StatusUnauthorized)
		_, _ = w.Write([]byte(`{"error":"bad key"}`))
	})
	...
	apiKey := "test-key"
	err := newGiteeAIForTest(srv.URL).CheckConnection(&APIConfig{ApiKey: &apiKey})
	if err == nil || !strings.Contains(err.Error(), "401") {
		t.Errorf("expected 401 propagated, got %v", err)
	}
}

func TestGiteeAIBaseURLForRegionUnknown(t *testing.T) {
	m := newGiteeAIForTest("http://unused")
	apiKey := "test-key"
	region := "missing"
...
func TestGiteeAIEmbedReturnsNoSuchMethod(t *testing.T) {
	model := "x"
	_, err := newGiteeAIForTest("http://unused").Embed(&model, []string{"a"}, &APIConfig{}, nil)
	if err == nil || !strings.Contains(err.Error(), "no such method") {
		t.Errorf("Embed: want 'no such method', got %v", err)
	}
}

func TestGiteeAIAudioOCRReturnNoSuchMethod(t *testing.T) {
	m := newGiteeAIForTest("http://unused")
	model := "x"
