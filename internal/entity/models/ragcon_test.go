...
func TestRAGconChatNoReasoning(t *testing.T) {
	srv := newRAGconServer(t, "/chat/completions", func(t *testing.T, body map[string]interface{}, w http.ResponseWriter) {
		_ = json.NewEncoder(w).Encode(map[string]interface{}{
			"choices": []map[string]interface{}{{
...
func TestRAGconChatPropagatesHTTPError(t *testing.T) {
	srv := newRAGconServer(t, "/chat/completions", func(t *testing.T, body map[string]interface{}, w http.ResponseWriter) {
...
func TestRAGconStreamSplitsReasoning(t *testing.T) {
	srv := newRAGconSSEServer(t, "/chat/completions",
		`data: {"choices":[{"index":0,"delta":{"role":"assistant"}}]}`+"\n"+
...
func TestRAGconStreamHonorsFinishWithoutDelta(t *testing.T) {
...
func TestRAGconStreamFailsWithoutTerminal(t *testing.T) {
	srv := newRAGconSSEServer(t, "/chat/completions",
...
func TestRAGconStreamRejectsMalformedFrame(t *testing.T) {
	srv := newRAGconSSEServer(t, "/chat/completions",
...
func TestRAGconStreamSurfacesUpstreamError(t *testing.T) {
	srv := newRAGconSSEServer(t, "/chat/completions",
...
func TestRAGconListModelsRequiresAPIKey(t *testing.T) {
	_, err := newRAGconForTest("http://unused").ListModels(&APIConfig{})
	if err == nil || !strings.Contains(err.Error(), "api key is required") {
		t.Errorf("expected api-key error, got %v", err)
	}
}

func TestRAGconCheckConnectionDelegatesToListModels(t *testing.T) {
	srv := newRAGconServer(t, "/models", func(t *testing.T, _ map[string]interface{}, w http.ResponseWriter) {
		_ = json.NewEncoder(w).Encode(map[string]interface{}{
...
func TestRAGconCheckConnectionPropagatesError(t *testing.T) {
	srv := newRAGconServer(t, "/models", func(t *testing.T, _ map[string]interface{}, w http.ResponseWriter) {
		w.WriteHeader(http.StatusUnauthorized)
		_, _ = w.Write([]byte(`{"error":"bad key"}`))
	})
	...
	apiKey := "test-key"
	err := newRAGconForTest(srv.URL).CheckConnection(&APIConfig{ApiKey: &apiKey})
	if err == nil || !strings.Contains(err.Error(), "401") {
		t.Errorf("expected 401 propagated, got %v", err)
	}
}

func TestRAGconBaseURLForRegionUnknown(t *testing.T) {
...
func TestRAGconEmbedReturnsNoSuchMethod(t *testing.T) {
	model := "x"
